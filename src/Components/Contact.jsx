...
const Contact =()=>{
    ...
    const onSubmit = async (e) => {
        ...
        if (data.success) {
            setLoading(false);
      setStatus("success");   // shows "Submitted"

        setTimeout(() => {
            e.target.reset();     // clear form
            setStatus("");        // back to "Submit"
        }, 1000);
        ...
        <div id="contact" className="max-w-full overflow-hidden py-20 md:py-28">
            ...
            <div className="px-4 lg:px-20 my-6 flex flex-col items-center">
                ...
                <form onSubmit={onSubmit} className="w-full lg:w-3/5 bg-(--contact-bg-color) rounded-lg px-8 flex flex-col gap-4 py-6 justify-start">
                    ...
                    <input name="name" type="text" required id="name" placeholder="Enter your name" className="bg-(--contact-box-color) w-full h-12 rounded-md border-0 outline-0 px-4 text-(--primary-text-color)" />
                    <label htmlFor="email" className="text-(--secondary-text-color) text-lg tracking-wider">Email</label>
                    <input name="email" type="email" required id="email" placeholder="Enter your Email" className="bg-(--contact-box-color) w-full h-12 rounded-md border-0 outline-0 px-4 text-(--primary-text-color)" />
